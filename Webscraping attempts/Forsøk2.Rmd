...
Må ha en metode som går inn i alle underkategori html filene for å få ut statistikken, før vi da kan lagre all data i om statistikken i en strukturert oversikt. 
```{r}
...
for (i in 1:length(liste)){
  statistikk_2 <- read_html(liste[i]) %>% 
  html_nodes('div[class='ssb-card']') %>%
  html_elements('a') %>% html_attr('href') %>% 
  str_c('https://ssb.no', .)
  #again with the nodes
  statistikk[[i]]<-statistikk_2
  #tester bare ut
}
"""
tmp <- list()
statistikk_liste <- list()
for (i in 1:length(hovedkategori_navn)) { 
  liste <- underkategorier[[i]]
  tmp <- list()
  for (i in 1:length(liste)){
    statistikk <- read_html(liste[j]) %>% 
    html_nodes('div[class="ssb-card"]') %>%
    html_elements("a") %>% html_attr("href") %>% 
    str_c("https://ssb.no", .)
    #again with the nodes
